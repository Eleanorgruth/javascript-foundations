...
describe('Medusa', function() {
  ...
  it('should have a name', function() {
    ...
    var eleanorVar = new Medusa("Eleanor");

    assert.instanceOf(eleanorVar, Medusa);
    // instantiate another Medusa object with someone else's name as an argument
    var sydneyVar = new Medusa("Sydney");

    assert.instanceOf(sydneyVar, Medusa);
    // assert that the first medusa's name is your name
    assert.equal(eleanorVar.name, "Eleanor")

    // assert that the second medusa's name is the other name
    assert.equal(sydneyVar.name, "Sydney")
...
//We have 3 person object instances, named victim1, victim2, and victim3
    medusa.gazeAtVictim(victim1);
    medusa.gazeAtVictim(victim2);
    medusa.gazeAtVictim(victim3);
    medusa.gazeAtVictim(victim4);
//the gazeAtVictim method is being called to create new class Statue insantiaions using the person.name as a argument
  ...
  it('should let the first victim go free when stoning the 4th', function() {
    ...
    var victim3 = new Person('Scott');
    var victim4 = new Person('Bob');
// these variables are data for the test to check
